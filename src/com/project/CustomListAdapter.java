...
public class CustomListAdapter extends ArrayAdapter<PlaceInfo> implements
	...
	private class ItemFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			for (int i = 0; i < count; i++) {
				filterableString = list.get(i).getPlaceText();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		Log.v("Position", PlaceItem.getPlaceToken());
		holder.txtTitle.setText(PlaceItem.getPlaceText());

		String url = PlaceItem.getPlaceUrl();

		if (holder.imgIcon != null) {
			// new ImageDownloaderTask(holder.imgIcon, position, url).execute();
